...
class Game {
    ...
    */
    ...
    roundToTwo (n) {
        return Math.ceil(n / 2.0) * 2
    }

    addSPoint (x, y) {
        return {
            'x': this.roundToTwo(x) - 6,
            'y': this.roundToTwo(y) - 6,
            'w': 12,
            'h': 12,
            'colour': 'red'
        }
    ...
    updateShapeControls (c) {
        if (!c.squares) {
            ...
            case 'p': // circle & arc
                c.squares.push(this.addSPoint(c.x, c.y))
                c.squares.push(this.addSPoint(c.x + c.rad, c.y + c.rad))
                c.squares.push(this.addSPoint(c.x, c.y + c.rad2))
                break
            case 'f': // polygon
                c.squares.push(this.addSPoint(c.x, c.y))
    ...
    initMouse (c) {
        ...
        c.addEventListener('mousedown', e => {
            ...
            self.processDown(this.roundToTwo(x), this.roundToTwo(y))
        }, false)
        ...
        c.addEventListener('mouseup', e => {
            ...
            var x = e.clientX - r.left
            var y = e.clientY - r.top
            self.processUp(this.roundToTwo(x), this.roundToTwo(y))
        }, false)
        ...
        c.addEventListener('mousemove', e => {
            ...
            var x = e.clientX - r.left
            var y = e.clientY - r.top
            self.processMove(this.roundToTwo(x), this.roundToTwo(y))
        }, false)
    }
    ...
    initButtons () {
        ...
        $('#sbox').on('click', () => {
            this.currentShape = {
                ...
                y: 100,
                placed: false,
                colour: this.currentColourIndex
            }
            this.dragging = false
            this.mouseMode = MODES.DRAW
            this.setDrawCursor()
        ...
        $('#scircle').on('click', () => {
            ...
            if (synth !== undefined) {
                ...
                let text = 'Wolf pack to the back!'
                let utterance = new SpeechSynthesisUtterance(text)
                utterance.pitch = 0.4 // 0 - 2
                utterance.rate = 1.0 // 0 - 2
        ...
        $('#reps').on('click', () => {
            let v = 'r,4,50,50'
            this.addToList(v, 'rep start')
        })
        $('#repe').on('click', () => {
            let v = 'r,0'
            this.addToList(v, 'rep end')
        })
    ...
    updateCommand () {
        ...
        if (shape) {
            ...
            case 'h':
            case 'j':
                com += `${shape.x},${shape.y},${shape.width},${shape.height},${shape.colour}`
                break
            case 'i':
            case 'm':
                ...
                break
            case 'p':
                com += `${shape.x},${shape.y},${shape.rad},${shape.rad2}`
                break
            case 'a':
    ...
    setCommand (shape, mx, my) {
        ...
        case 'm':
            ...
            break
        case 'p':
            com += `${mx},${my},${shape.rad},${shape.rad2}`
            break
        case 'a':
                ...
                if (this.currentShape && this.currentShape.selected !== null) {
                    ...
                    if (i > 0) {
                        ...
                        case 'm': // circle
                            ...
                            break
                        case 'p': // ellipse
                            let a2 = mx - this.currentShape.x
                            let b2 = my - this.currentShape.y
                            switch (i) {
                            case 1:
                                this.currentShape.rad = Math.floor(a2)
                                break
                            case 2:
                                this.currentShape.rad2 = Math.floor(b2)
                                break
                            }
                            break
                        case 'd': // line
    ...
    convertDataToShape (data) {
        ...
        commands.forEach(c => {
            ...
            case 'n': // round rect
                ...
                break
            case 'e': // backbox, backbar
            case 'g': // backbox, backbar
            case 'j': // backbox, backbar
                this.currentShape.width = l[2]
                this.currentShape.height = l[3]
                break
            case 'd': // line
